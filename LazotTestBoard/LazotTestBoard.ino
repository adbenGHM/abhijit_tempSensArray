...
#define TEMPERATURE_SET_POINT_DEFAULT (35)

#define TEMPERATURE_MIN_POINT (20)
...
void SetSettingsToDefault(void)
{
  ...
  for (i = 0; i < (SENSOR_COUNT - 1); i++)
  {
    if (gSensorInstances[i].mode <= SENSOR_MODE_NONE || gSensorInstances[i].mode >= SENSOR_MODE_MAX)
    {
...
static void CheckForTemperatureTriggers(void)
{
  ...
  for (i = 0; i < (SENSOR_COUNT - 1); i++)
  {
    ...
    else if (gSensorInstances[i].tempRead == NAN || gSensorInstances[i].tempRead <= 0)
    {
...
{
  return (gBoardNum <= SENSOR_BOARD_NONE || gBoardNum >= SENSOR_BOARD_MAX) ? false : true;
}
...
void loop()
{
  ...
  if (gSensorInstances[SENSOR_COUNT - 1].tempRead > 350) {
    digitalWrite(SOLENOID_PIN, LOW);
  } else  if (gSensorInstances[SENSOR_COUNT - 1].tempRead < 300) {
